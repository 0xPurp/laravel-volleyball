...
@section('content')
    <section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800 mt-20">
        <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Team Update</h2>
                ...
                <div>
                    ...
                    <select name="continent_id" id="continent"
                        ...
                        @foreach ($continents as $continent)
                            <option value="{{ $continent->id }}" {{$continent->id == $equipe->continent_id ? 'selected' : null}}>{{ $continent->continent }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
